...
// morgan 미들웨어 -> HTTP 요청에 대한 로그 기록
const morgan = require('morgan');

// cookieParser 미들웨어 -> 클라이언트가 보낸 쿠키 파싱
const cookieParser = require('cookie-parser')

// express-session 미들웨어-> 세션 관리
...
// express.static -> 정적 파일들을 제공하는 라우터 역할
// public 디렉토리 안에 있는 파일들을 제공 -> 직접 읽어서 전송하지 않아도 됨.
app.use('/', express.static(path.join(__dirname, 'public')));

// 요청의 본문에 있는 데이터 해석 -> req.body 객체를 만들어주는 미들웨어
...
//요청에 동봉된 쿠키를 해석 -> req.cookies 객체에 저장
...
app.use(session({
    ...
    // saveUninitialized -> 세션에 자장할 내역기 없더라도 처음부터 세션을 생성할지 설정
    // secret -> 안전하기 쿠키를 전송하려면 쿠키에 서명을 추가해야 함. 쿠키에 서명할 때 secret 값이 필요
    // cookie -> 세션 쿠키에 대한 설정
        // httpOnly: true -> 클라이언트에서 쿠키를 확인할 수 없음.
        // secure: false -> https 가 아닌 환경에서도 사용할 수 있음.
    // name: : 'session-cookie' -> 세션쿠키의 이름을 'session-cookie'로 지정
...
const upload = multer({
    ...
    // 업로드 될 파일의 최대 크기 제한 -> 5MB(5 * 1024 * 1024byte)로 설정
...
// 루트 경로('/')로 GTE 요청이 오면 실행
    ...
    // threw net Error -> 에러를 발생시키고, 이를 등록된 에러 처리 미들웨어로 전달
